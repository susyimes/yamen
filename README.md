...
如果你现在已经看完 README，想知道 **Yamen 到底怎么接进 OpenClaw**，就记住一句话：

> **不要改 OpenClaw，把 Yamen 当成一套运行规则挂在 OpenClaw 上。**

也就是：

- **OpenClaw 负责能力**：会话、工具、消息、子 agent
- **Yamen 负责制度**：门房分流、县令拍板、主簿成案、快手执行、典史复核

---

### 最简单的接法

先不要搞后端、数据库、前端，也不要一开始就把所有角色自动化。

直接这样接：

1. 用 `yamen` 仓库作为你的工作流规范目录
2. 让 **主会话** 先兼任“门房 + 县令”
3. 需要时再调用其它角色
...
#### 第 1 步：把 Yamen 仓库放进 OpenClaw 工作区
确保这些文件存在：

- `AGENTS.md`
- `agents/menfang/SOUL.md`
- `agents/xianling/SOUL.md`
...
这些文件不是摆设，它们就是 Yamen 的“制度说明书”。

---

#### 第 2 步：先不要单独建一堆入口，先让主会话按规则办事
最开始最推荐的做法是：
...
也就是说，一开始你不用真的拆成 5 个长期在线 agent。

先把 **规则跑顺**，比先把 **角色拆满** 更重要。

---

#### 第 3 步：需要哪个角色，再调用哪个角色
在 OpenClaw 里，对应关系可以先这么理解：

- **主簿**：负责写任务单、整理说明、生成回禀草稿
- **快手**：负责真正执行任务
- **典史**：只在高风险任务时做复核
...
##### 直办
```text
用户 → 主会话判断为直办 → 直接答 / 或交给快手执行
```

##### 立案
```text
用户 → 主会话判断为立案 → 主簿出任务单 → 快手执行 → 主会话回禀
```

##### 复核案
```text
用户 → 主会话判断为复核案 → 主簿 / 快手处理 → 典史复核 → 主会话回禀
```
...
在 OpenClaw + Yamen 里，最简做法就是：

1. 主会话按门房规则识别：这是案件，不是闲聊
2. 主会话按县令规则判断：这是 `filed`，不需要典史
3. 调主簿：生成一个简版任务单
4. 调快手：按任务单去修改
5. 主会话汇总后回给用户
...
> **把 Yamen 当成 OpenClaw 主会话的一套轻量办事制度。**
